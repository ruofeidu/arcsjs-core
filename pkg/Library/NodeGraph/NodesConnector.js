...
async update(inputs, state, {service}) {
  const {pipeline, nodeTypes, selectedNode} = inputs;
  ...
  if (pipeline?.nodes) {
    if (this.pipelineChanged(pipeline, state.pipeline)
      ...
      // const results = await service({msg: 'updateNodes', data: {pipeline, selectedNode}});
      // return results;
      const newPipeline = await service({msg: 'updateNodes', data: {pipeline}});
      const recipes = await service({kind: 'RecipeService', msg: 'PipelineToRecipes', data: {pipeline: newPipeline}});
      return {
